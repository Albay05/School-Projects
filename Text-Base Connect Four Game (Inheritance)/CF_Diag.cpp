...
namespace cfgame {
    ...
    bool ConnectFourDiag::check_win(int row, int col, CellState player_state) {
    ...
    ConnectFourDiag::ConnectFourDiag(int row, int col) {
    ...
    ConnectFourDiag::ConnectFourDiag(std::fstream& file) {
        readBoard(file); // readBoard function called to handle process
    }


    ConnectFourDiag::~ConnectFourDiag() { // Deleting the cell pointers
        for(int i = 0; i < this->height; i++) {
            delete [] gameboard[i];
        }
